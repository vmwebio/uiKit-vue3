<script setup>
// Определяем события, которые будут отправляться родительскому компоненту
  ...
  const handleClick = (event) => {
    if (props.group) {
    // Если это группа чекбоксов, отправляем событие 'updateCheckboxGroup'
      emits('updateCheckboxGroup', {optionId: props.id, checked: event.target.checked})
    } else {
    // Иначе отправляем событие 'update:checked'
...
<template>
<!-- Определяем контейнер для чекбокса или переключателя -->
  <div :class="[{'switch-container': type === 'switch'}]">
    <input
      ...
      <!-- Отображаем метку для чекбокса или переключателя -->
